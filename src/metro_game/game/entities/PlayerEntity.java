...
public class PlayerEntity extends GameEntity {
	...
	public PlayerEntity(Context context, float x, float y) {
		...
		float width = 1.0f;
		float height = 1.0f;
		m_shape = addShape(new RectShape(x, y, width, height, 0.0f));
		m_body = addBody(new BoxBody(true, x, y, width, height));
		m_clickPos = null;
		m_aimShape = addShape(new RectShape(0, 0, 0.0f, 0.1f, 0.0f));
		m_aimShape.setVisible(false);
	...
	public void update(double delta) {
		...
		m_aimShape.getSize().x = aimLength;
		...
		m_aimShape.setRotation((float) Math.toDegrees(m_aimAngle));
		m_aimShape.getPosition().set(newPos);
